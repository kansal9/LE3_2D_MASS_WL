 ...
 */
  void readVisibilityMask(fs::path& workdir, fs::path& catalogName, std::vector < std::vector < double> >& data);
  ...
  * @param[in]     <data> <std::vector<std::vector<double> > > data of mask in vectors (ra, dec & weight)
 */
  void getMaskData(fs::path& workdir, fs::path& InFile, std::vector<std::vector<double> >& data);
 ...
 */
  void getMaskData(const std::string& filename, std::vector<std::vector<double> >& data);
 ...
 */
  std::pair<Healpix_Map<double>, Healpix_Map<double> > changeResolution(const std::string& filename);
