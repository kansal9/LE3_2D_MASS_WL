...
static Elements::Logging logger = Elements::Logging::getLogger("VisibilityMask_test");
//-----------------------------------------------------------------------------
...
//BOOST_AUTO_TEST_SUITE (VisibilityMask_test)
...
BOOST_AUTO_TEST_CASE( readVisibilityMask_test ) {
  ...
  try
  {
    //auto filename = Elements::getAuxiliaryPath("LE3_2D_MASS_WL_UTILITIES/vMask.xml");
...
BOOST_AUTO_TEST_CASE( getMaskDataCatalogXMLException_test ) {
  logger.info() << "-- VisibilityMask: getMaskDataCatalogXMLException_test";
    ...
    BOOST_CHECK_THROW(vk.readVisibilityMask(dataDirectoryPath, inputFileName, testData), Elements::Exception);
}
