...
{
    if (maskFile.string().empty())
    {
        throw Elements::Exception() << "Input Mask filename is not found";
    }
    fs::path datadir { workdir / "data" };
    std::string inputFileName;
    if (checkFileType(datadir / maskFile, signFITS))
    {
        if (false == fs::exists(datadir / maskFile))
        {
            throw Elements::Exception() << "Input data product "
                  << datadir / maskFile << " not found";
        }
    ...
    { //Case 2: When input file is in XML format
        if (false == fs::exists(workdir / maskFile))
        {
            throw Elements::Exception() << "Input data product "
                    << workdir / maskFile << " not found";
        }
        ...
        std::vector<std::vector<double> >& data)
{
    ...
    if (false == checkFileType(workdir / InFile, signFITS))
    {
        if (checkFileType(workdir / InFile, signXML))
        {
            logger.info(" Input file is in XML format..");
            logger.info() << "Using file " << workdir / InFile
                          << " as DM input product";
            if (fileHasField((workdir / InFile).native(), "DpdTwoDMassVisibilityMask"))
            {
                logger.info() << "Input File type: FITS: Visibility Mask";
                // Read filename of the FITS file from the input XML file
                DmInput inp;
                inp.readXmlFile(workdir / InFile);
                // Get fits filename
                m_inputFile = (datadir / inp.getCatalogFilename()).string();
...
void VisibilityMask::getMaskData(const std::string& filename,
        std::vector<std::vector<double>>& data)
{
    if (checkFileType(fs::path(filename), signFITS))
    {
        ...
        {
            std::pair<double, double> radec = getIndex2RaDec(mapPair.first,
                    id_pix);
        ...
        const std::string& filename)
{
    ...
    else
    {
        ...
        else
        {
            ...
            logger.info()
                    << "The current resolution is same as desired resolution . .";
        }
    }

    return std::pair<Healpix_Map<double>, Healpix_Map<double> >(outMapE,
            outMapB);
}
