...
static Elements::Logging logger = Elements::Logging::getLogger("Projection_test");
//-----------------------------------------------------------------------------
...
BOOST_AUTO_TEST_CASE( example_test ) {
  ...
  for (int i=0; i<36; i++)
  {
    ...
    for (int j=0; j<18; j++)
    {
      ...
      std::pair<double, double> myXY = Projection.getGnomonicProjection(ra, dec, ra0, dec0);
      std::pair<double, double> myBackRaDec = Projection.getInverseGnomonicProjection(myXY.first, myXY.second, ra0, dec0);
...
BOOST_AUTO_TEST_CASE( example_test2 ) {
   logger.info() << "--Projection: example_test2";
      ...
      Projection Projection;
      std::pair<double, double> myXYMin = Projection.getGnomonicProjection(raMin, decMin, ra0, dec0);
      std::pair<double, double> myBackRaDecMin = Projection.getInverseGnomonicProjection(myXYMin.first, myXYMin.second, ra0, dec0);
      std::pair<double, double> myXYMax = Projection.getGnomonicProjection(raMax, decMax, ra0, dec0);
      std::pair<double, double> myBackRaDecMax = Projection.getInverseGnomonicProjection(myXYMax.first, myXYMax.second, ra0, dec0);
