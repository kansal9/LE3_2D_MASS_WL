...
void readSphericalParameterFile(const fs::path& ParamFile,
                                SphericalParam &params,
                                CatalogData& cat)
{
    logger.info() << "Using Paramfile: " << ParamFile << " as DM input product";
    //check file is of XML format
    if (checkFileType(ParamFile.native(), signXML))
    {
        if (fileHasField(ParamFile.native(),
                "DpdTwoDMassParamsConvergenceSphere"))
        {
            logger.info() << "Parameter file is for Spherical Convergence";
            params.readConvSphereXMLFile(ParamFile.native(), cat);
            logger.info()
                    << "Done reading Spherical Convergence Parameter file";
        }
    }
    else
    {
        logger.info()
                << "Parameter file is not provided or not in XML format . .";
    }
}
...
void getFilter(std::vector<double>& array, double lc, int m_nlmax)
{
    for (int l = 0; l <= m_nlmax; l++)
    {
...
std::vector<Healpix_Map<double> > transformBspline_hp(Healpix_Map<double>& map,
        int m_nbScales)
{
    int m_npix = map.Npix();
