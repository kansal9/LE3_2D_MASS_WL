...
static Elements::Logging logger = Elements::Logging::getLogger("LE3_2D_MASS_WL_SphericalMassMapping");
...
void getSphericalConvergenceMap (std::pair<Healpix_Map<double>, Healpix_Map<double> >& mapPair,
    std::pair<Healpix_Map<double>, Healpix_Map<double> >& kappaPair, SphericalParam &SphericalParam) {
...
class LE3_2D_MASS_WL_SphericalMassMapping : public Elements::Program {
...
public:
  ...
  options_description defineSpecificProgramOptions() override {
   ...
   options.add_options()
   ("inGamma", po::value<string>()->default_value(""), "input shear Map E and B mode in fits format ");

   // input product (Density Map)
   options.add_options()
   ("inGalCount", po::value<string>()->default_value(""),
                      "Galaxy count Map which conatins number of Galaxies per pixel in fits format ");

   // input product (convergence Map)
   options.add_options()
   ("inKappa", po::value<string>()->default_value(""), "input kappa Map E and B mode in fits format ");

   // output json file for convergance Maps
   options.add_options()
   ("outputKappa", po::value<string>()->default_value(""),
   ...
   options.add_options()
   ("outputShear", po::value<string>()->default_value(""), "Output convergence map E mode in BinTable Format");
   // output XML for convergance Map E and B mode
   options.add_options()
   ("outputKappaXML", po::value<string>()->default_value(""), "Output XML product for Spherical convergence map");
    return options;
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(outputKappa.string().empty()) {
    outputKappa = fs::path("EUC_LE3_WL_SphereConvergence_" + getDateTimeString() + ".json");
  }
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   readSphericalParameterFile((workdir/ParamFile), SphParam);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
 // Creating Shear Map from Convergence Map (Perform inverse mass mapping KS)
////////////////////////////////////////////////////////////////////////////////////////////////////////
if((false == inputKappa.string().empty())) {
  logger.info("#");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(outputGamma.string().empty()) {
    outputGamma = fs::path("EUC_LE3_WL_K2Shear_"  + getDateTimeString() + ".fits");
  }
////////////////////////////////////////////////////////////////////////////////////////////////////////
 // Writing Kappa 2 shear Fits files
////////////////////////////////////////////////////////////////////////////////////////////////////////
  logger.info("# Writing Spherical Gamma Map got from Convergence Map");

   SphericalIO.write_Map((datadir/outputGamma).native(), k2shearPair.first, "GAMMA1");
   SphericalIO.write_Map((datadir/outputGamma).native(), k2shearPair.second, "GAMMA2");
} else {
////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // Generate the output FITS filenames
    outputKappaFits = fs::path("EUC_LE3_WL_NoisySphereConvergence_" + getDateTimeString() + ".fits");

 // Writing output Fits files
  logger.info("# Writing Spherical Noisy Convergence Map");
  SphericalIO.write_Map((datadir/outputKappaFits).native(), kappaPair.first, "KAPPA_E");
  SphericalIO.write_Map((datadir/outputKappaFits).native(), kappaPair.second, "KAPPA_B");
  outfile << outputKappaFits.filename();
////////////////////////////////////////////////////////////////////////////////////////////////////////
 // Get De-Noisy Kappa Maps
////////////////////////////////////////////////////////////////////////////////////////////////////////

   if (fabs(SphParam.getSigmaGauss())>0.001){//Apply gaussian filter on the map
     applyGaussianFilter_hp(mapE, SphParam.getSigmaGauss()); //this will update contents by applying filter
     applyGaussianFilter_hp(mapB, SphParam.getSigmaGauss()); //this will update contents by applying filter
     outputKappaFits.clear();
     mapPair = std::make_pair (mapE, mapB);
     getSphericalConvergenceMap (mapPair, kappaPair, SphParam);
   // Generate the output FITS filenames
     outputKappaFits = fs::path("EUC_LE3_WL_DenoisedSphereConvergence_" + getDateTimeString() + ".fits");
     outfile << ",";
   // Writing output Fits files
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    fs::path out_xml_product_file_name = args["outputKappaXML"].as<string>();
    Euclid::WeakLensing::TwoDMass::DmOutput dm;
    if ((out_xml_product_file_name.string()).empty() == true) {
      out_xml_product_file_name = fs::path("outputKappaXML_" + getDateTimeString() + ".xml");
    }
    fs::path file = workdir / out_xml_product_file_name;

    std::vector<fs::path> filenames = read_filenames(workdir, outputKappa);
    ...
    auto product = initProduct<dpd::le3::wl::twodmass::out::convergencesphere::dpdTwoDMassConvergenceSphere,
			                        ...
			                        int>(product_type, file, SphParam.getNResamples());
    std::vector<fs::path> GalCountfilename = read_filenames(workdir, inputGalCount);
    ...
    for (size_t i=0; i<filenames.size(); i++) {
      ...
      if ((filenames[i].string()).find("NoisySphereConvergence") != std::string::npos) {
        dm.createNoisedSphereXml(product, mapOut);
      }
      if ((filenames[i].string()).find("DenoisedSphereConvergence") != std::string::npos) {
        dm.createDenoisedSphereXml(product, mapOut);
      }
    }
    writeProduct<dpd::le3::wl::twodmass::out::convergencesphere::dpdTwoDMassConvergenceSphere>(product, file);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  std::chrono::duration<double> elapsed_seconds = SphMassMapping_end-SphMassMapping_start;
  std::time_t SphMassMapping_end_time = std::chrono::system_clock::to_time_t(SphMassMapping_end);
  logger.info() << "finished Program at " << std::ctime(&SphMassMapping_end_time);
...
MAIN_FOR(LE3_2D_MASS_WL_SphericalMassMapping)
