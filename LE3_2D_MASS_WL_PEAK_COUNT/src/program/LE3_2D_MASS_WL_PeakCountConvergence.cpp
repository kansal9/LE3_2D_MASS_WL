...
using LE3_2D_MASS_WL_PEAK_COUNT::WaveletPeakCount;
...
static Elements::Logging logger = Elements::Logging::getLogger("LE3_2D_MASS_WL_PeakCountConvergence");

class LE3_2D_MASS_WL_PeakCountConvergence : public Elements::Program {
...
public:
  ...
  // input parameter file
   options.add_options()
   ("paramPeakConvergence", po::value<string>()->default_value(""), "input configuration file");
   ...
   // input
   options.add_options()
   ("inputConvMap", po::value<string>()->default_value(""), "input Convergence Map");
   // output file for Peak Catalog
   options.add_options()
   ("outputPConvCatalog", po::value<string>()->default_value(""), "Output Peak Count Catalog in fits");
   options.add_options()
   ("outputPConvCatalogXML", po::value<string>()->default_value("outputPConvCatalogXML.xml"),
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   fs::path fileParamPeakConv {args["paramPeakConvergence"].as<std::string>()};
   fs::path logPath (args["logdir"].as<string>());
   fs::path inputConvMap {args["inputConvMap"].as<std::string>()};
   fs::path outputPConvCatalog {args["outputPConvCatalog"].as<std::string>()};
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
  // check whether parameter file (XML format) is for wavelet based Peak Count Algorithm or not
////////////////////////////////////////////////////////////////////////////////////////////////////////
  PeakParam params;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
   if ((outputPConvCatalog.string()).empty() == true) {
     outputPConvCatalog = fs::path ("EUC_LE3_WL_PeakCatalog_Wavelet_" + getDateTimeString() + ".fits");
   }
////////////////////////////////////////////////////////////////////////////////////////////////////////
  //Peak Count Wavelet algorithm
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ConvergenceMap *m_conv;
    m_conv = new ConvergenceMap((workdir/inputConvMap).native());
     logger.info()<<"PixelSize: "<<m_conv->getPixelSize();
    // Then create a PeakCountAlgo object
    WaveletPeakCount myPeakCounter(*m_conv, params);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
   auto out_xml_peak_convergence_catalog =args["outputPConvCatalogXML"].as<std::string>();
    // Create a DMOutput object
    DmOutput dm;
    std::string catfile = (workdir / fs::path(out_xml_peak_convergence_catalog)).string();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   std::chrono::duration<double> ConvPeak_elapsed_seconds = ConvPeak_end-start_ConvPeakTime;
   //std::time_t ConvPeak_end_time = std::chrono::system_clock::to_time_t(ConvPeak_end);
   //logger.info() << "finished Program at " << std::ctime(&ConvPeak_end_time);
   logger.info() << "elapsed time: " << ConvPeak_elapsed_seconds.count() << "s";
...
MAIN_FOR(LE3_2D_MASS_WL_PeakCountConvergence)
